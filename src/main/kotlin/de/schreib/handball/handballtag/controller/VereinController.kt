...
class VereinController(
    ...
    @GetMapping("all")
    fun getAllVerein(): List<Verein> =
        vereinRepository.findAll().filter { it.name != PLATZHALTER_VEREIN_NAME }.sortedBy { it.name }

    @GetMapping("{id}/mannschaften")
    fun getAllVereinMannschaften(
        @PathVariable
        id: Long
    ): List<Mannschaft> {
